...
const RaceFeatures = ({ selectedRace }: RaceFeaturesProps) => {
  ...
  return (
    <div
      ref={containerRef}
      className="race-feature flex relative flex-wrap gap-1.5 justify-center lg:justify-start"
    >
      {selectedRace.features.map((feature, index) => {
        ...
        return (
          <div key={index} className="relative group">
            ...
            <IconFeature imageSrc={feature.image} isActive={isActive} />

            <div
              onClick={() => setActiveIndex(isActive ? null : index)}
              className="absolute top-0 left-0 w-full h-full z-10 lg:hidden"
            ></div>
          </div>
        );
      })}
